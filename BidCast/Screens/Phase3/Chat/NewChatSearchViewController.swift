//
//  NewChatSearchViewController.swift
//  BidCast — iOS parity Phase 3c (2026-04-22)
//
//  "New message" flow — search users via POST /api/user/searching (shared
//  with Social search VC). Tap to start a conversation.
//

import UIKit

final class NewChatSearchViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate {
    ...
    private let onPick: (SearchUserEntry) -> Void

    init(onPick: @escaping (SearchUserEntry) -> Void) {
        self.onPick = onPick
    ...
    func searchBarTextDidChange(_ searchBar: UISearchBar, text: String) {}
