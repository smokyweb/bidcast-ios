//
//  CompanyUserProfileScreen.swift
//  imperium
//
//  Created by JAM-E-221 on 29/01/25.
//

import SwiftUI

struct CompanyUserProfileScreen: View {
    @Environment(\.presentationMode) var presentationMode
    
    @State var userDetail: CompanyUserResponse = CompanyUserResponse()
        ...
        ZStack {
            VStack(alignment: .leading, spacing: 0, content: {
                HeaderWithImageTitle(
                    ...
                    userName: .constant(userDetail.first_name ?? ""),
                    userImg: .constant(userDetail.profile_image?.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? ""))
...
//                                Divider()
                        ...
                        .refreshable(action: {
                            ...
                            isLoading = true
                            self.viewModal.getSubCompanyDetails()
                        })
                        Spacer()
...
//                    NotificationCenter.default.addObserver(self, selector: #selector(handleData(_:)), name: Notification.Name("EventsDataGoogle"), object: nil)
            ...
            CusNavLink(doNavigate: $navigateToUpdateProfile, destination: CreateSubCompanyScreen(comeFromuserList: false, comeFromuserProfile: true, userId: $userId))
    ...
    func handleSuccess() {
        if viewModal.requestType == "GetCompany" {
            if let response = viewModal.companyResponseDetails {
