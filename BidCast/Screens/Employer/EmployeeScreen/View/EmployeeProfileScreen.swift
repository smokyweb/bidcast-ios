...
//    @State var request: PerformJobActionRequest = PerformJobActionRequest(status: 0, job_id: 0, user_id: 0)
...
//                                    let status = "3"
//                                    let job_id = job.id ?? 0
//                                    let user_id = userDetail.id ?? 0
//                                    isLoading = true
//                                    self.viewModal.performJobAction(parameter: PerformJobActionRequest(status: status, job_id: job_id, user_id: user_id))
//                                })
//                            
//                            .padding(.all)
//                            .padding(.vertical, 20)
                    ...
                    HStack {
                        ...
                        Button(action: {
                            ...
                            if UserDefaults.EmployerRightSwipe == ""{
                                ...
                                alertType = .sheetType(icon: .alert, title: "Right Swipe Not Found", message: "No more right swipe left, Do you want to purchase right swipe?", primaryBtnText: "Yes", secondaryBtnText: "No", sheetThemeColor: .pinkBtn)
...
//            }
        ...
        .onAppear(perform: {
            isLoading = true
            self.viewModal.getEmployeeDetail(employeeId: "\(employeeId ?? 0)", jobId: String(job.id ?? 0))
            observe()
        })
    }
    ...
    func handleSuccess() {
        ...
        } else if viewModal.requestType == "GetJobDetails" {
            if let response = viewModal.jobDetailResponse {
                if response.status == "success" {
                    job = response.data
                }
            }
