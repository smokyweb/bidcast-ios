//
//  ExampleVideoResume.swift
//  imperium
//
//  Created by JAM-E-221 on 18/10/24.
//
...
struct ExampleVideoResume: View {
    @Environment(\.presentationMode) var presentationMode
    @EnvironmentObject private var appRootManager: AppRootManager
    
    @State private var player : AVPlayer?
    
    @State var title: String = "Example Video Resume"
    ...
    var body: some View {
        VStack(spacing: 0) {
            PrimaryHeader(title: title, leadingImgArr: [.sideArrow], onClickLeading: { _ in
                if isMainFlow {
                    ...
                    if isNextClicked {
                        ...
                        DispatchQueue.main.async {
                            player = AVPlayer(url: getMediaURL(url: videoURL))
                            player!.play()
                        }
                        withAnimation(.easeInOut) {
                            title = "Example Video Resume"
                            isNextClicked = false
                        }
                    } else {
